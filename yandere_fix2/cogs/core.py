...
class Core(commands.Cog):
    ...
    async def help(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="👁️ Yandere Simulator", value="`/character` — Look up any character from the game with full profile, traits & image", inline=False)
    ...
    @app_commands.command(name="setchannel", description="Set the channel for ambient Yandere messages.")
    ...
    async def setchannel(self, interaction: discord.Interaction, channel: discord.TextChannel = None):
        ...
        data._db.setdefault("guild_config", {}).setdefault(str(interaction.guild.id), {})["ambient_channel"] = target.id
        data.save()
        await interaction.response.send_message(
            f"🌸 *All my thoughts will find their way to {target.mention} from now on. 👁️*",
            ephemeral=True,
        )

    @app_commands.command(name="clearchannel", description="Reset ambient channel — falls back to first available.")
    @app_commands.checks.has_permissions(manage_channels=True)
    @app_commands.guild_only()
    async def clearchannel(self, interaction: discord.Interaction):
        data._db.get("guild_config", {}).get(str(interaction.guild.id), {}).pop("ambient_channel", None)
        data.save()
        await interaction.response.send_message("🌸 *I'll find my own way to reach you. 💕*", ephemeral=True)

    @app_commands.command(name="testambient", description="Send a test ambient message to the configured channel now.")
    @app_commands.checks.has_permissions(manage_channels=True)
    @app_commands.guild_only()
    async def testambient(self, interaction: discord.Interaction):
        import sys, random as _random
        main = sys.modules["__main__"]
        channel = main._get_ambient_channel(interaction.guild)
        if channel:
            await channel.send(_random.choice(main.AMBIENT_MESSAGES))
